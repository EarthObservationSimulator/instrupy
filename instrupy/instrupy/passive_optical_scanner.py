...
class PassiveOpticalScanner(Entity):
    ...
    def calculate_number_of_signal_electrons(opWav_m, bw_m, bbT_K, apDia_m, opTrns, QE, tObs_JDUT1,
        ...
        Lint = PassiveOpticalScanner.radianceWithEarthAsBlackBodyRadiator(opWav_m, bw_m, bbT_K, obsIncAng_rad, considerAtmosLoss) + PassiveOpticalScanner.radianceWithEarthAsReflector(opWav_m, bw_m, tObs_JDUT1, obs_pos_km, tar_pos_km, pixelArea_m2, considerAtmosLoss)
    ...
    def radianceWithEarthAsReflector(opWav_m, bw_m, tObs_JDUT1, obs_pos_km, tar_pos_km, obs_area_m2, considerAtmosLoss):
        ...
        '''
        Lint = PassiveOpticalScanner.planck_photon_integral_with_wavlen_dependent_atmos_loss_2((opWav_m - bw_m*0.5), (opWav_m + bw_m*0.5), Constants.SunBlackBodyTemperature, solar_inc_angle_rad, obsIncAng_rad, considerAtmosLoss)
    ...
    def planck_photon_integral_with_wavlen_dependent_atmos_loss_1(wav_low_m, wav_high_m, bbT_K, obs_zen_rad, considerAtmosLoss):
        ...
        if(considerAtmosLoss is True):
            ...
            for indx in range(0,len(wav_m)-1):
                Lint = Lint + 0.5*(trEff[indx]+trEff[indx+1])*(PassiveOpticalScanner.planck_photon_integral(wav_m[indx], bbT_K) - PassiveOpticalScanner.planck_photon_integral(wav_m[indx+1], bbT_K))
        else:
            Lint = (PassiveOpticalScanner.planck_photon_integral(wav_high_m, bbT_K) - PassiveOpticalScanner.planck_photon_integral(wav_low_m, bbT_K))
        return Lint
    ...
    def planck_photon_integral_with_wavlen_dependent_atmos_loss_2(wav_low_m, wav_high_m, bbT_K, sun_zen_rad, obs_zen_rad, considerAtmosLoss):
        ...
        if(considerAtmosLoss is True):
            ...
            for indx in range(0,len(wav_m)-1):
                Lint = Lint + (0.5*(trEff_SunPath[indx]+trEff_SunPath[indx+1]))*(0.5*(trEff_ObsPath[indx]+trEff_ObsPath[indx+1]))*(PassiveOpticalScanner.planck_photon_integral(wav_m[indx], bbT_K) - PassiveOpticalScanner.planck_photon_integral(wav_m[indx+1], bbT_K))
        else:
            Lint = (PassiveOpticalScanner.planck_photon_integral(wav_high_m, bbT_K) - PassiveOpticalScanner.planck_photon_integral(wav_low_m, bbT_K))
